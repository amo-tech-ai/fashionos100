...
- **Two Booking Systems:** This document covers the full 13-phase wizard. For the 7-step ecommerce booking, see `05-design-prompts.md` Section 19
- **Schema Reference:** Uses `shoots` table (not `briefs`) - aligns with `03-shoot-schema.md`
...
export default function BookingWizard() {
  ...
  const updateBrief = async (updates: Partial<BriefState>) => {
    ...
    try {
      await supabase.functions.invoke('manage-brief', {
        body: {
          action: 'update',
          briefId: brief.id,
          data: updates
