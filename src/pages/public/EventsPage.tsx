...
import { FEATURED_EVENT, CATEGORIES, Event } from '../../data/mockEvents';
...
export const EventsPage: React.FC = () => {
  ...
  // Fetch real events
  const { events: realEvents, loading: eventsLoading } = useEvents({ status: 'published' });

  // Map DB events to UI Event type
  const mappedEvents: Event[] = useMemo(() => {
    return realEvents.map(e => ({
      id: e.id,
      title: e.title,
      category: 'Event', // Default categorization or derive from DB
      timing: new Date(e.start_time) > new Date() ? 'Upcoming' : 'Past',
      date: new Date(e.start_time).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }),
      time: new Date(e.start_time).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' }),
      location: e.venue?.name || 'TBD',
      image: e.featured_image_url || 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?auto=format&fit=crop&q=80&w=600',
      tags: [new Date(e.start_time) > new Date() ? 'Upcoming' : 'Past'],
      price: e.ticket_tiers && e.ticket_tiers.length > 0 ? `From $${Math.min(...e.ticket_tiers.map(t => t.price))}` : 'Free',
      capacity: 'Open'
    }));
  }, [realEvents]);
  ...
  const handleAISearch = async () => {
    ...
    setIsAiLoading(true);
    try {
        // Construct context from currently loaded events
        const eventsContext = mappedEvents.map(e => ({
  ...
  const filteredEvents = useMemo(() => {
    return mappedEvents.filter(event => {
      // AI Search Filter
      // Note: aiMatches contains IDs which might be strings (UUIDs) or numbers. 
      // We need to ensure type compatibility.
      if (aiMatches !== null && !aiMatches.includes(event.id as any)) return false;
      ...
      // Manual Search Fallback (if AI not used)
      if (!aiMatches && searchQuery) {
  ...
  }, [mappedEvents, typeFilter, priceFilter, statusFilter, aiMatches, searchQuery]);
                    ...
                    <div className="relative shrink-0">
                       ...
                       {showCalendar && (
                          <>
                             ...
                             {/* Desktop Popover */}
                             <div className="hidden md:block absolute top-full left-0 mt-2 z-50">
                             ...
                             {/* Mobile Bottom Sheet */}
                             <div className="fixed bottom-0 left-0 right-0 bg-white rounded-t-[2rem] p-4 z-[70] md:hidden animate-in slide-in-from-bottom duration-300 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] border-t border-gray-100 pb-safe">
                                <div className="w-12 h-1 bg-gray-200 rounded-full mx-auto mb-6" />
                                <div className="flex justify-between items-center mb-4 px-2">
                                    <h3 className="font-serif font-bold text-xl text-gray-900">Select Dates</h3>
                                    <button onClick={() => setShowCalendar(false)} className="p-2 bg-gray-100 hover:bg-gray-200 rounded-full text-gray-500 transition-colors">
                                      <X size={20}/>
                                    </button>
                                </div>
