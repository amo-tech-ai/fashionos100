...
export const HomePage: React.FC = () => {
  // Refs for performant direct DOM manipulation
  const column1Ref = useRef<HTMLDivElement>(null);
  const column2Ref = useRef<HTMLDivElement>(null);

  useEffect(() => {
    // Optimization: Disable parallax on mobile to improve performance and prevent jitter
    ...
    const handleScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          // Parallax Logic:
          // Column 1 moves slightly down (slower scroll feel)
          if (column1Ref.current) {
            column1Ref.current.style.transform = `translateY(${scrolled * 0.06}px)`;
          }
          // Column 2 moves slightly up (faster scroll feel) for contrast
          if (column2Ref.current) {
            column2Ref.current.style.transform = `translateY(-${scrolled * 0.08}px)`;
          }
          ticking = false;
        });
        ticking = true;
          ...
          <div className="lg:col-span-7 order-2 relative">
            <FadeIn direction="left" delay={200}>
              <div className="grid grid-cols-2 gap-4 md:gap-6 pb-12">
                <div 
                  ref={column1Ref}
                  className="space-y-4 md:space-y-6 mt-12 lg:mt-24 will-change-transform transition-transform duration-75 ease-out"
                >
                  {/* Fixed Image 1 */}
                ...
                <div 
                  ref={column2Ref}
                  className="space-y-4 md:space-y-6 will-change-transform transition-transform duration-75 ease-out"
                >
