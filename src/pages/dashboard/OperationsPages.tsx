...
export const DashboardActivations: React.FC = () => {
  const { id: eventId } = useParams<{ id: string }>(); // Event Context Support
  ...
  useEffect(() => {
    const fetchActivations = async () => {
      ...
      let query = supabase
        ...
        .select('*, event_sponsor:event_sponsors(event_id, sponsor:sponsor_profiles(name), event:events(title))')
        .order('created_at', { ascending: false });
        
      const { data } = await query;
      
      if (data) {
          // Filter in memory for complex join logic or do it in query if structure allows
          // event_sponsor.event_id access via filter is tricky with standard Supabase client joins deeply nested
          // Doing in-memory for MVP since volume is low
          let filtered = data;
          if (eventId) {
              filtered = data.filter((a: any) => a.event_sponsor?.event_id === eventId);
          }
  ...
  return (
    <div className="animate-in fade-in duration-700 pb-20 font-sans">
      <div className="flex flex-col md:flex-row justify-between items-end mb-10">
        <div>
          <h1 className="text-4xl font-serif font-bold text-gray-900 mb-2">{eventId ? 'Event Activations' : 'Brand Activations'}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {activations.length === 0 && (
            <div className="col-span-full text-center text-gray-400 py-12 border-2 border-dashed border-gray-100 rounded-2xl">
                No activations found for this context.
            </div>
        )}
      </div>
    </div>
