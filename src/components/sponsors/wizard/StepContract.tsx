...
import { FileText, Upload, Wand2, Download } from 'lucide-react';
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn("PDF Generation function not reachable, using text fallback.");
      ...
      alert("Could not generate PDF. Downloading text version instead.");
      // Fallback
      const blob = new Blob([data.terms], { type: 'text/plain' });
      const url = window.URL.createObjectURL(blob);
      triggerDownload(url, `${data.sponsorName}-Contract-Draft.txt`);
