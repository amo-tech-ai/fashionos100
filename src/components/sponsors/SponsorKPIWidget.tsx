...
export const SponsorKPIWidget: React.FC<SponsorKPIWidgetProps> = ({ sponsors, loading, onAiTrigger }) => {
  const metrics = useMemo(() => {
    ...
    const activePartners = new Set(
      sponsors
        .filter(s => ['Signed', 'Paid', 'Contacted', 'Negotiating'].includes(s.status))
        .map(s => s.sponsor_id)
    ).size;
      ...
      <FadeIn delay={100} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all cursor-pointer group" onClick={onAiTrigger}>
        ...
        <div className="mt-3 space-y-2">
           <div className="flex h-2 rounded-full overflow-hidden bg-gray-100">
              ...
              <div style={{ width: `${(metrics.pipelineCounts['Lead'] || 0) / metrics.totalDeals * 100}%` }} className="bg-blue-400" title="Lead" />
           </div>
           <div className="flex justify-between text-[10px] text-gray-500 font-medium">
              ...
              <span className="flex items-center gap-1"><div className="w-1.5 h-1.5 rounded-full bg-blue-400"/> {metrics.pipelineCounts['Lead'] || 0} New</span>
           </div>
        </div>
      </FadeIn>
    </div>
