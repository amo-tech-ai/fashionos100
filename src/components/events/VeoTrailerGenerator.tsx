...
export const VeoTrailerGenerator: React.FC<VeoTrailerGeneratorProps> = ({ featuredEvent, brandColors }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
        ...
        const startResp = await aiService.generateMedia({
            action: 'start',
            prompt: promptText,
        ...
        if (!startResp.success || !startResp.data?.operationName) {
            throw new Error('Failed to start video generation');
        }
        
        const { operationName } = startResp.data;
        ...
        // Polling loop (max 3 minutes for video)
        const startTime = Date.now();
        while (!isDone && Date.now() - startTime < 180000) {
            await new Promise(resolve => setTimeout(resolve, 6000));
            
            const pollResp = await aiService.generateMedia({
                action: 'poll',
                operationName
            });
            
            if (pollResp.success && pollResp.data) {
                isDone = pollResp.data.done;
                downloadUri = pollResp.data.uri;
            }
        ...
        if (downloadUri) {
            ...
            // We use direct fetch here because we need blob response, 
            // but we go through our edge function proxy to keep API key hidden
  ...
  const handleShare = () => {
    if (navigator.share && videoUrl) {
      navigator.share({
          ...
          text: 'Check out this event trailer created with FashionOS!',
